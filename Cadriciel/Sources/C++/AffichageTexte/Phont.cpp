 ...
 * @author Philippe Proulx
 */
Phont::Phont() : loaded_(false), cl_base_(0), count_(0), chars_(NULL) {
}
 ...
 * @author Philippe Proulx
 *
 * @param width		La longueur du viewport
 ...
 * @return		Succès
 */
bool Phont::print(GLuint width, GLuint height, GLuint vp_x, GLuint vp_y, bool keep_gl, const char* format, ...) const {
	if (!loaded_) {
		return false;
	}
	...
	glLoadIdentity();
	gluOrtho2D(0.0, (GLdouble) width - 1.0, 0.0, (GLdouble) height - 1.0);
	...
	glPushMatrix();
		...
		glTranslated((GLdouble) vp_x, (GLdouble) vp_y, 0.0);
		glListBase(cl_base_);
