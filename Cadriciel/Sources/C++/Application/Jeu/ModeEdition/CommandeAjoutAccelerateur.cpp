...
CommandeAjoutAccelerateur::CommandeAjoutAccelerateur(const vue::Vue& vue, Carte& carte) : vue_(vue), carte_(carte)
{
	type_ = ACCELERATEUR;
	mouseClicked_ = false;
}

CommandeAjoutAccelerateur::~CommandeAjoutAccelerateur()
{
}

void CommandeAjoutAccelerateur::executer(int mouseX, int mouseY, int boutton, int touche)
{
	...
	else if (boutton == -1 && mouseClicked_)
	{
		if((mouseX >= (clickedPositionX_ - 3) && mouseX <= (clickedPositionX_ + 3)) && (mouseY >= (clickedPositionY_ - 3) && mouseY <= (clickedPositionY_ + 3)))
		{
			...
			{
				Accelerateur* a = new Accelerateur();
				a->setPositionActuelle(positionVirtuelle);
				carte_.ajouterElement(a);
			}
		}
...
bool CommandeAjoutAccelerateur::validerPosition(Vecteur3& position)
{
...
void CommandeAjoutAccelerateur::afficheCurseurRouge(const Vecteur3& positionSouris) const {
