...
CommandeAjoutPortail::CommandeAjoutPortail(const vue::Vue& vue, Carte& carte) : vue_(vue), carte_(carte)
{
...
CommandeAjoutPortail::~CommandeAjoutPortail()
{
}

void CommandeAjoutPortail::executer(int mouseX, int mouseY, int boutton, int touche)
{
	...
	if (boutton == 1 && mouseClickRelacher_ && mousePremierClicked_) {
		...
		if((mouseX >= (clickedPositionX_ - 3) && mouseX <= (clickedPositionX_ + 3)) && (mouseY >= (clickedPositionY_ - 3) && mouseY <= (clickedPositionY_ + 3))) {
			// Convertir la position de la souris du premier clic (changement de systeme de coordonnees)
			Vecteur3 lastPositionVirtuelle;
			vue_.convertirClotureAVirtuelle(lastMouseX_, lastMouseY_, lastPositionVirtuelle);

			// Convertir la position de la souris du deuxieme clic (changement de systeme de coordonnees)
			...
			if (validerPosition(lastPositionVirtuelle) && validerPosition(positionVirtuelle))
			{
				...
				Portail* p2 = new Portail();

				p1->setPositionActuelle(lastPositionVirtuelle);
				carte_.ajouterElement(p1);

				p2->setPositionActuelle(positionVirtuelle);
				carte_.ajouterElement(p2);
...
bool CommandeAjoutPortail::validerPosition(Vecteur3& position)
{
...
void CommandeAjoutPortail::afficheCurseurRouge(const Vecteur3& positionSouris) const {
